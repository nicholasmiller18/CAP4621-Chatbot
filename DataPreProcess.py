...
if __name__ == "__main__":
    ...
    oldMaxID = 1
    print("Converting strings to number arrays for the posts, comments, and dictionary")
    for x in range(totalPairs):
        ...
        for y in range(oldMaxID,d.maxID):
            c.execute("INSERT INTO dict('index','word') VALUES (?,?);",(y,d.getWord(y)))
            oldMaxID = d.maxID
        connection.commit()

        #print out every 1,000 pairs for sanity check
        if x % 1000 == 0 and x != 0:
            print("Inserted {} pairs into 3 tables".format(x))
            print("Dictionary check:")

            result = c.execute("SELECT word FROM dict LIMIT 1 OFFSET {};".format(x))
            result = c.fetchone()
            asdf = result[0]
            print("{} -> {}".format(x,asdf))